...
RUN echo "=== POPULATING SYSROOT WITH SPIRV-HEADERS, SPIRV-TOOLS, AND GLSLANG ===" && \
    ...
    ls -laR "$INSTPREFIX/include" || true; \
    \
    echo "→ Copying glslang libraries"; \
    cp -a "$INSTPREFIX/lib"/libglslang* "$SYSROOT/usr/lib/" || echo "⚠ no glslang libs found"; \
    ls -la "$INSTPREFIX/lib" || true; \
    ls -la "$SYSROOT/usr/lib" || true; \
    \
    ...
    echo "# Autogenerated glslangConfig.cmake" > "$SHIM_GLS"; \
    echo "set(GLSLANG_INCLUDE_DIR \"$INSTPREFIX/include\")" >> "$SHIM_GLS"; \
    echo "set(GLSLANG_LIBRARY_DIR \"$INSTPREFIX/lib\")" >> "$SHIM_GLS"; \
    echo "set(GLSLANG_BINARY_DIR \"$INSTPREFIX/bin\")" >> "$SHIM_GLS"; \
    echo "set(GLSLANG_FOUND TRUE)" >> "$SHIM_GLS"; \
